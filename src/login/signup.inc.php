...
if (isset($_POST['submit'])) {/* Checks if the user actually clicked the submit button */
	...
	include_once '../src/globals/dbh.inc.php';/* Database connection */
	...
	if (empty($first) || empty($last) || empty($email) || empty($uid) || empty($pwd)) {
		header("Location: ../../public/signup.php?signup=empty");/* Takes user back with an error message*/
		exit();

	}else {
		//Check if input characters are valid
		if (!preg_match("/^[a-zA-Z]*$/", $first)  || !preg_match("/^[a-zA-Z]*$/", $last)) {//Checking if variables src/ does not include this pattern 
			header("Location: ../../public/signup.php?signup=invalid");/* Takes user back */
			exit();

		}else {
			//Check if email is valid
			if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
				header("Location: ../../public/signup.php?signup=email");
				exit();			
			}else {
	...
	header("Location: ../../public/signup.php");/* Takes user back */
	exit();
}