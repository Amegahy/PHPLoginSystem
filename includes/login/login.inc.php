...
if (isset($_POST['submit'])) {
	...
	include '../dbh.inc.php';//Includes the php file which makes the connection
	...
	if (empty($uid) || empty ($pwd)) {
		...
		if ($resultCheck < 1) {//No results so the user does not exist
			header("Location: ../../index.php?login=incorrect");//Don't let users know the error and guess values
			exit();	
		...
		header("Location: ../../index.php?login=NotPressed");
		exit();
	}